...
namespace Layout_2._1
{
    public class Custom
    {
        ...
        public static void ErrorHandle(Exception ex, HttpResponse Response, HttpContext context)
        {
            // Determine the appropriate status code based on the exception
            int statusCode;
            if (ex is HttpException)
            ...
            switch (statusCode)
            {
                case 200:
                    break;
                case 404:
                    context.Server.Transfer("~/Error Handler/Error404.aspx");
                    break;
                case 500:
                    context.Server.Transfer("~/Error Handler/Error500.aspx");
                    break;
                default:

                    context.Server.Transfer("~/Error Handler/Error.aspx");
                    break;
            }
        }
    }
